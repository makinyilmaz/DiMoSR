...
class SWTLoss(nn.Module):
    def __init__(self, loss_weight_ll=0.01, loss_weight_lh=0.01, loss_weight_hl=0.01, loss_weight_hh=0.01, reduction='mean', overall_weight=None, filter_name=None):
        ...
        if self.filter_name == None:
            print("FILTER NAME IS NOT DEFINED")
    ...
    def forward(self, pred, target):
        ...
        return self.overall_weight * (loss_subband_LL + loss_subband_LH + loss_subband_HL + loss_subband_HH)

@LOSS_REGISTRY.register()
class SWTRatio(nn.Module):
    def __init__(self):
        super(SWTRatio, self).__init__()
    ...
    def forward(self, pred):
        ...
        wavelet_sr  = sfm(pred)[0] ##36 channel output reduced to 1 as if it was y channel
        # LL_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,0:1, :, :][0]), dim=-1),dim=-1)[0]
        # LH_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,1:2, :, :][0]), dim=-1),dim=-1)[0]
        # HL_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,2:3, :, :][0]), dim=-1),dim=-1)[0]
        # HH_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,3:, :, :][0]), dim=-1),dim=-1)[0]

        LL_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,0:1, :, :]), dim=-1),dim=-1)
        LH_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,1:2, :, :]), dim=-1),dim=-1)
        HL_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,2:3, :, :]), dim=-1),dim=-1)
        HH_sr   = torch.sum(torch.sum(torch.square(wavelet_sr[:,3:, :, :]), dim=-1),dim=-1)

        swt_ratio = LL_sr/(LH_sr + HL_sr + HH_sr) ##if ratio larger --> flat region
        # print(swt_ratio)
        return swt_ratio
